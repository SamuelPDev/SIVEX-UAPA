...
const EditStudent: React.FC = () => {
  ...
  useEffect(() => {
    getStudents().then(res => {
      const s = res.data.find(x => x.id === originalId);
      if (!s) {
        alert("Estudiante no encontrado");
        return navigate("/gestionar-estudiantes");
      }
      setForm(s);
      setLoading(false);
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.id.trim()) {
      return alert("El ID no puede quedar vacío");
    }
    await updateStudent(originalId!, form);
  ...
  if (loading) {
    return (
      <div className="page-content">
        <Container fluid><p>Cargando estudiante…</p></Container>
      </div>
    );
  }
  ...
  return (
    <div className="page-content">
      <Container fluid>
        <Row><Col lg={12}><h5 className="mb-4">EDITAR ESTUDIANTE</h5></Col></Row>
